...
def display_query_result(result):
    """
    Displays SQL query result nicely with table, row count and CSV download
    """
...
def run_query(username, password, sql):
    start = time.time()
    r = requests.post(
        f"{API_BASE}/execute",
        json={
...
def run_admin_query(token, sql, db_selected):
    start = time.time()
    r = requests.post(
...
if st.session_state.authenticated and st.session_state.role == "user":
    ...
    sql = st_ace(
        ...
        height=200,
        auto_update=True,
        keybinding="vscode",   # Enables Ctrl+Enter / Cmd+Enter shortcuts
    ...
    # Run Query Button
    if st.button("Run Query", disabled=not can_run) or st.session_state.get("trigger_run_user"):
        st.session_state["trigger_run_user"] = False
        r, elapsed = run_query(st.session_state.username, st.session_state.password, sql)
        if r.status_code == 200:
            log_query(sql, "success", elapsed)
            st.success(f"Executed in {elapsed}s")
            display_query_result(r.json()["result"])
        else:
            log_query(sql, "error", elapsed)
            st.error(r.json().get("detail"))

    # Ctrl+Enter or Cmd+Enter shortcut listener
    if st.experimental_get_query_params().get("run") == ["1"]:
        st.session_state["trigger_run_user"] = True
        st.rerun()
...
if st.session_state.authenticated and st.session_state.role == "admin":
    ...
    with tabs[0]:
        ...
        if st.button("Run Admin Query", disabled=not can_run):
            r, elapsed = run_admin_query(st.session_state.admin_token, sql, db_selected)
            log_query(
    ...
    with tabs[1]:
        ...
        db = st.selectbox("Select database", user_dbs, key="admin_db_explorer")
        if db:
            r = requests.get(
